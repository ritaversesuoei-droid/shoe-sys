...
export function judgeShift(
  ...
  // --- 1日の拘束時間 ---
  if (restraintMin != null) {
    if (restraintMin > dr.max_min) {
      items.push({
        type: "restraint",
        severity: "violation",
        message: `拘束時間が上限(${hhmm(dr.max_min)})を超過`,
        actualMin: restraintMin,
        thresholdMin: dr.max_min,
      });
    } else if (restraintMin > dr.extended_threshold_min) {
  ...
  if (restPeriodMin != null) {
    if (restPeriodMin < rp.min_floor_min) {
      items.push({
        type: "rest_period",
        severity: "violation",
        message: `休息期間が下限(${hhmm(rp.min_floor_min)})未満`,
        actualMin: restPeriodMin,
        thresholdMin: rp.min_floor_min,
      });
    } else if (restPeriodMin < rp.principle_min) {
      items.push({
        type: "rest_period",
        severity: "warning",
